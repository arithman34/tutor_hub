...
# Duration in minutes computed from start/end timestamps
...
async def dashboard(request: Request, db: AsyncSession = Depends(get_db), user: User = Depends(get_current_user_from_cookie)):
    ...
    # ── FINANCIAL ────────────────────────────────────────────────────────────────
    revenue_alltime = float(await db.scalar(
        select(func.sum(Payment.amount))
    ) or 0)
    revenue_this_month = float(await db.scalar(
        select(func.sum(Payment.amount)).where(Payment.payment_date >= this_month_start)
    ) or 0)
    revenue_last_month = float(await db.scalar(
    ...
    top_by_revenue = [
        ...
        for r in (await db.execute(
            select(
                ...
                func.sum(_mins / 60.0 * Student.hourly_rate).label("rev"),
            )
            .select_from(User)
            .join(Session, Session.user_id == User.id)
            .join(Student, Session.student_id == Student.id)
    ...
    tutors_with_balance = [
        {
            ...
            "owed": round(float(r.mins or 0) / 60 * float(r.payout_hourly_rate or 0), 2)
                    if r.payout_type == PayoutType.hourly else None,
        }
        for r in (await db.execute(
            select(
                User.first_name, User.last_name, User.payout_type, User.payout_hourly_rate,
    ...
    new_students = (await db.execute(
        select(Student).options(joinedload(Student.user))
        .where(Student.created_at >= this_month_start)
        .order_by(Student.created_at.desc())
    )).unique().scalars().all()

    students_no_active_tutor = (await db.execute(
        select(Student).join(User, Student.user_id == User.id)
        .where(Student.is_active == True, User.is_active == False)
    )).scalars().all()

    # ── ALERTS ───────────────────────────────────────────────────────────────────
    # Outstanding balance = sum of negative payee balances (payees who owe more than they've paid)
    ...
    paid_rows = (await db.execute(
        select(Payment.payee_id, func.sum(Payment.amount).label("paid"))
    ...
    outstanding_balance = round(sum(
        max(0, _cost_map.get(pid, 0) - _paid_map.get(pid, 0))
        for pid in all_payee_ids
    ), 2)

    tutors_no_payout = (await db.execute(
        select(User).where(User.role == UserRole.tutor, User.is_active, User.payout_type.is_(None))
    )).scalars().all()
    ...
    _month_lit = _text("'month'")
    _month_expr = func.date_trunc(_month_lit, Student.created_at)
    growth_rows = (await db.execute(
        select(
            _month_expr.label('month'),
            func.count().label('count'),
        )
        .where(Student.created_at >= twelve_months_ago)
    ...
    recent_sessions = (await db.execute(
        select(Session).options(joinedload(Session.student), joinedload(Session.user))
        .order_by(Session.created_at.desc()).limit(10)
    )).unique().scalars().all()

    recent_payments = (await db.execute(
        select(Payment).options(joinedload(Payment.payee))
        .order_by(Payment.payment_date.desc()).limit(5)
    )).scalars().all()

    new_tutors = (await db.execute(
        select(User).where(User.role == UserRole.tutor, User.created_at >= this_month_start)
